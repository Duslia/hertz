...
func (routerNode *RouterNode) DyeGroupName() error {
	...
	hook := func(layer int, node *RouterNode) error {
		...
		if node.MiddleWare == "" {
			...
			node.MiddleWare = strings.ToLower(pname)
		}
...
func (pkgGen *HttpPackageGenerator) updateRegister(pkg, rDir, pkgName string) error {
	...
	if !bytes.Contains(file, []byte(insertImport)) {
		...
		subIndexsReg := regRegisterV3.FindSubmatchIndex(file)
		if len(subIndexsReg) != 2 || subIndexsReg[0] < 1 {
			return fmt.Errorf("wrong format %s: insert-point '%s' not found", string(file), insertPointPatternOld)
		}

		bufReg := bytes.NewBuffer(nil)
		bufReg.Write(file[:subIndexsReg[1]])
		bufReg.WriteString("\n\t" + insertReg)
		bufReg.Write(file[subIndexsReg[1]:])
